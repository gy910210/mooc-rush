...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (IDandRatingsTuple[0], (length, sum(IDandRatingsTuple[1]) / float(length)))
...
# TODO: Replace <FILL IN> with appropriate code
myModel = ALS.train(trainingRDD.union(validationRDD), bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
...
trainingAvgRating = float(trainingRDD.map(lambda (x, y, z) : z).reduce(lambda x, y : x + y)) / float(trainingRDD.count())
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x : x not in set([x[1] for x in myRatedMovies]))
