...
def tokenize(string):
    ...
    """
    return [token for token in re.split(split_regex, string.lower()) if len(token) > 0 and token not in stopwords]
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.flatMap(lambda pair : pair[1]).count()
...
def findBiggestRecord(vendorRDD):
    ...
    """
    # print vendorRDD.count()
    return sorted(vendorRDD.collect(), key=lambda x : len(x[1]), reverse=True)
...
def tf(tokens):
    ...
    for token in dic.keys():
        dic[token] = float(dic[token]) / float(n);
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {key : (tfs[key] * idfs[key]) for key in tfs.keys()}
    return tfIdfDict
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda x : (x[1] + ' ' + x[0], float(x[2])))

trueDupsRDD = (sims
              .join(goldStandard))
trueDupsCount = trueDupsRDD.count()

avgSimDups = float(trueDupsRDD.map(lambda x : x[1][0]).reduce(lambda x, y : x + y)) / float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda x : x[1][1] == None))
avgSimNon = float(nonDupsRDD.map(lambda x : x[1][0]).reduce(lambda x, y : x + y)) / float(nonDupsRDD.count())
...
def fastCosineSimilarity(record):
    ...
    s = 0
    for key in tokens:
        s = s + amazonWeightsBroadcast.value[amazonRec][key] * googleWeightsBroadcast.value[googleRec][key]
