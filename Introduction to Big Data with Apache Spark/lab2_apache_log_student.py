...
uniqueHosts = hosts.reduceByKey(lambda x, y : x + y).map(lambda s : s[0])
...
dayHostCount = dayGroupedHosts.map(lambda s : (s[0], len(s[1])))
...
groupedByDay = dayAndHostTuple.groupByKey().mapValues(list)

sortedByDay = groupedByDay.map(lambda s : (s[0], len(s[1]) / len(set(s[1]))))
